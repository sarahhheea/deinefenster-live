...
**Seitenteil-Varianten für alle 14 Modelle (Compositing-Ansatz):**
Statt 14 × 3 (ST-L, ST-R, 2-Fl) × 2 (I+A) = 84 zusätzliche AI-Master wird ein **Pipeline-Compositing** eingesetzt:
- 1-Fl Modell-Master + generisches Seitenteil-Panel → automatisch zusammengesetzt
- Seitenteil = festes Glaselement im gleichen Rahmen/Farbe neben der Tür
- Benötigt: **2 generische Seitenteil-Elemente** (Innen + Außen) als AI-Master
- Pipeline compositet: Modell + Seitenteil-L, Modell + Seitenteil-R, Modell + beidseitig

**Haustür gesamt: 38 AI-Master** (8 generisch + 28 Modell-spezifisch + 2 Seitenteil-Elemente)
...
### 3.6 ROLLADEN (Optionsbilder — Gurt und Motor SEPARAT)

| # | Bild | Zweck |
|---|------|-------|
| 1 | Fenster + Gurt (Gurtband sichtbar) | Thumbnail Rolladen-Optionskarte |
| 2 | Fenster + Motor (nur Kasten, kein Gurt) | Thumbnail Rolladen-Optionskarte |
| 3 | Balkontür + Gurt | Thumbnail |
| 4 | Balkontür + Motor | Thumbnail |
| 5 | HST + Gurt | Thumbnail |
| 6 | HST + Motor | Thumbnail |

Diese sind Optionsbilder ohne Farbvarianten. Gurt und Motor sind zwei verschiedene Produkte mit verschiedenen Preisen.
**AI-Bilder: 6**
...
> **Optimierung:** Flips können statt als gespeicherte Dateien auch per CSS `transform: scaleX(-1)` zur Laufzeit erzeugt werden. Das spart ~1.575 Dateien und reduziert die Gesamtzahl auf ~5.181. Empfehlung: CSS-Flip für Haustüren, gespeicherte Flips nur für Fenster/Balkontür 1-Fl (wo die Flip-Anzahl überschaubar ist).

**Compositing-Bonus:** Seitenteil-Varianten der 14 Modelle werden durch Pipeline-Compositing erzeugt (kein extra AI nötig), liefern aber ~14 × 3 × 2 × 45 = ~3.780 zusätzliche Bilder.

**121 AI-Master + ~5.175 Farbvarianten + ~1.575 Flips = ~6.750 Bilder gesamt** (oder ~5.181 mit CSS-Flips).
...
10. Seitenteil-Compositing-Pipeline (Modell + Seitenteil-Element)
11. Pipeline-Farben für alle Modelle

### Phase 4: HST + Rolladen
12. HS, PSK, Slide Master (Innen + Außen)
13. Alle Rolladen-Varianten (Fenster, Balkontür, HST) — Gurt + Motor jeweils separat

### Phase 5: Balkontür 2-Fl + Feinschliff
14. Balkontür 2-Fl
15. Grau-Dichtung-Varianten
16. Qualitäts-Audit aller Bilder
...
- `scripts/flip-anschlag.py` — Horizontaler Flip für Anschlag-Varianten (Fenster/Balkontür 1-Fl)
- `scripts/composite-seitenteil.py` — Haustür-Modell + Seitenteil-Element zusammensetzen (L/R/beidseitig)
- `scripts/validate-bilder.py` — Größe, Farbe, Proportionen prüfen
- `scripts/generate-all.sh` — Orchestriert: Master → Compositing → Recolor → Flip → Validierung
...
## 10. Geklärte Fragen (Stand 2026-05-08)

1. **Gurt vs. Motor → SEPARATE MASTER.** Zwei verschiedene Produkte, zwei verschiedene Preise. Gurt zeigt sichtbares Gurtband am Rahmen. Motor zeigt nur Kasten ohne Gurtband.

2. **Haustür Seitenteil × Modell → JA, alle 14 Modelle.** Aber: Seitenteil = festes Glaselement neben der Tür, gleicher Rahmen. Die Tür selbst bleibt identisch. **Empfehlung: Compositing-Ansatz** statt 56 separate AI-Master. Pipeline nimmt 1-Fl Modell-Master + generisches Seitenteil-Element und setzt sie zusammen. Spart ~48 AI-Generierungen. Benötigte generische Elemente: Seitenteil-Panel (Glas + Rahmen, passend zu Fenster-/Türprofil).

3. **4-Flügel → Phase 2, wird gebraucht.** Schon in PROD_IMGS_VIEW vorhanden. Konstruktion: 4 Flügel nebeneinander in einem Rahmen, gleichmäßig aufgeteilt.

4. **Oberlicht + Rolladen → JA.** Rolladenkasten sitzt ÜBER dem gesamten Fenster inkl. Oberlicht (Aufsatzrolladen). Optik ändert sich: Kasten ist breiter/höher. Alle Oberlicht-Varianten brauchen Rolladen-Master.
