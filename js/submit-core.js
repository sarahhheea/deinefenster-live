...
(function (global) {
  ...
  // Mailversand läuft über den Cloudflare Worker (Resend): interne Mail
  // ans Team + automatische Bestätigung an den Kunden. Kein Web3Forms mehr.
  const TEAM_EMAIL = 'info@baustoffchrist.de';

  // Eindeutige Anfrage-Nummer: PREFIX-JJMMTT-XXXX (Datum + 4 Zufallszeichen).
  // Pro Anfrage praktisch nur einmal vergeben, gut dem Kunden zuordenbar.
  function genOfferId(prefix) {
    const d = new Date();
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
  ...
  <tr><td style="background:#fff;padding:0 36px 28px">
    ...
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      ...
      // ── Detail-Zeilen für die Kunden-Bestätigung — "edel & warm" Stil ──
      // Serifen-Titel + Hairline-Zeilen. configRows enthält keine € (Hausregel).
      // Die 'Produkt'-Zeile entfällt, da Menge + Name schon im Titel stehen.
      const kundenDetailsHtml = cart.map((item) => {
        const rows = configRows(item.config)
          .filter(([k]) => k !== 'Produkt')
          .map(([k, v]) =>
            `<tr><td style="padding:9px 0;border-bottom:1px solid #f0eadd;font-size:13px;color:#8a8273;width:44%;vertical-align:top;">${k}</td><td style="padding:9px 0;border-bottom:1px solid #f0eadd;font-size:13px;color:#1a2336;">${v}</td></tr>`
          ).join('');
        return `
        <div style="font-family:Georgia,'Times New Roman',serif;font-size:16px;color:#225eaa;margin:24px 0 2px;">${prodName(item.config.prod)} <span style="color:#b0a791;font-size:13px;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">· ${item.qty} Stück</span></div>
        <table width="100%" cellpadding="0" cellspacing="0">${rows}</table>`;
      }).join('');

      // ── Klartext-Fallback der internen Mail (falls HTML nicht rendert) ──
      ...
      const internalText = [
        `Neue Anfrage ${offerId} — DeineFenster.de`, '',
        `Name:    ${formData.name}`,
        `E-Mail:  ${formData.email}`,
        `Telefon: ${formData.phone || '–'}`,
        `Ort/PLZ: ${formData.ort}`,
        `Straße:  ${formData.strasse || '–'}`,
        `Wunsch-Liefertermin: ${formData.wunschLiefertermin || '–'}`,
        `Anmerkungen: ${formData.notiz || '–'}`,
        `Fotos/Skizzen: ${(uploadedImages || []).length}`, '',
        positionen, '', konfigDetail,
      ].join('\n');

      // ── Cloudflare Worker (Resend): interne Mail (schönes HTML) an das
      // Team + automatische Bestätigungsmail mit allen Eigenschaften an
      // den Kunden. Ein Dienst, kostenlos, kein Web3Forms mehr nötig. ──
      try {
        await fetch('https://deinefenster-email.sarahchrist.workers.dev', {
          method: 'POST',
      ...
      } catch (e) { console.warn('Mailversand fehlgeschlagen:', e); }
