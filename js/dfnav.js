...
(function(){
  ...
  function build(){
    ...
    a.className='df-cart-link'; a.href='/konfigurator.html?cart=1'; a.setAttribute('aria-label','Warenkorb ansehen');
  ...
  var isKonfig=/konfigurator/i.test(location.pathname||'');
  function inject(){
    if(isKonfig) return null;   // Konfig-Seite hat schon „Meine Artikel" — kein doppeltes Symbol
    ensureStyle();
